...
class GetCardInvoiceStatus
{
    ...
    public function getCardInvoiceStatus(array $params)
    {
        ...
        $this->epsUtli = new Utuls($this->is_test, "cardinvoices/{$card_invoice_no}/status", $this->token);

        if ($this->use_signature) {
            $params['signature'] = $this->epsUtli->computeSignature($params, $this->secret_word, 'status-card-invoice');
        }
